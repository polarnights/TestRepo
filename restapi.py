...
def update_user():
    if (
        ...
        or not "citizenship" in request.json
    ):
        abort(400)
    user = list(
        filter(lambda t: t["phone_number"] == request.json["phone_number"], users)
    )
    if not user:
        abort(404)
...
@app.route("/teremok/delete_user/<string:phone_number>", methods=["GET"])
@auth.login_required
