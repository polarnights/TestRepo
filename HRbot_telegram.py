...
def phone_handler(message, data):
    phone_number_text = message.text
    cursor = connection.cursor()
    try:
        cursor.execute('INSERT INTO user_info (phone_number) VALUES (%(phone)s)',
...
def callback_worker(call):
    ...
    if call.data == "Success2":
        bot.send_message(call.message.chat.id, text="Ответьте на 3 вопроса, чтобы узнать вас получше.\n")
        keyboard = types.InlineKeyboardMarkup()
    ...
    if call.data == "BadFinal" or call.data == "GoodFinal":
        phone_number = bot.send_message(call.message.chat.id,
                                      "Укажите Ваш номер телефона (без \"+\", начиная с 7, например: 79091234567)")
        bot.register_next_step_handler(phone_number, phone_handler, call.data)
